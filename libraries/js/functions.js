...
var deepLinkUpdater = function(){
    ...
    var share_pseudo_url = '?uid='+currentSceneUid+'&yaw='+currentYaw+'&pitch='+currentPitch+'&fov='+currentFov+'&title='+currentSceneTitle;
...
var activeSpotManagement = function(id){
    ...
    if(jQcurrentActiveSpot.length > 0){
        var rawId = jQcurrentActiveSpot.attr('id'),
            ...
            snapshotUrl = SV_assetsRootUrl+'img/poi/snapshots/'+id+'.jpg',
...
var shareView = function(){
    ...
    jQuery
        .ajax({
            method: "POST",
            url: "https://api.cloudinary.com/v1_1/doufg1wen/image/upload",
            data: { file: imageData, upload_preset: "go8eiuyx" }
        })
        // .ajax({
        //     method: "POST",
        //     url: "upload.php",
        ...
        .done(function(data) {
            ...
            // temp = temp[1].split('.');
            // var hash = temp[0];
            var redirecting_host = 'https://simple-redirect.olivier3lanc.repl.co/';
            var project_host = encodeURIComponent(window.location.origin+window.location.pathname);
            var screenshotUrl = data.secure_url;
            var screenshotUrl_trunc = screenshotUrl.replace('https://', '');
            var share_get_parameters = '?url='+project_host+'&uid='+currentSceneUid+'&yaw='+currentYaw+'&pitch='+currentPitch+'&fov='+currentFov+'&title='+encodeURIComponent(currentSceneTitle)+'&tn='+screenshotUrl_trunc;
            console.log(share_get_parameters);
            var share_pseudo_url = redirecting_host+share_get_parameters;
